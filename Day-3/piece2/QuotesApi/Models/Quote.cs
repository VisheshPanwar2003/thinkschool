...
public class Quote
{
    ...
    private Quote(string author, string text, DateTimeOffset createdAt, int ownerId)
    {
        Author = author;
        Text = text;
        CreatedAt = createdAt;
    ...
    public static Result<Quote> Create(string author, string text, DateTimeOffset createdAt, int ownerId)
    {
        ...
        if (ownerId <= 0)
            return Result<Quote>.Failure(new DomainError("A valid quote owner is required."));

        return Result<Quote>.Success(new Quote(author, text, createdAt, ownerId));
    }
