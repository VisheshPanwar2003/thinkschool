...
public static class AuthEndpoints
{
    ...
    private static string GenerateJwt(User user, IConfiguration config)
    {
        ...
        var token = new JwtSecurityToken(config["Jwt:Issuer"], config["Jwt:Audience"], claims, expires: DateTime.UtcNow.AddSeconds(config.GetValue<int>("Jwt:ExpiresInSeconds")), signingCredentials: creds);
    ...
    public static void MapAuthEndpoints(this WebApplication app)
    {
        ...
        group.MapPost("/login", async ([FromBody] LoginRequest request, AppDbContext db, IConfiguration config, CancellationToken ct) =>
        {
        ...
        group.MapPost("/refresh", async ([FromBody] RefreshRequest request, AppDbContext db, IConfiguration config, ILogger<Program> logger, CancellationToken ct) =>
        {
            ...
            return Results.Ok(new LoginResponse(newAccessToken, newRefreshToken, 900));
        });
